...
function UnitInfo:init(unit, u_key, manager)
	...
	elseif Utils:IsInstanceOf(u_base, CopBase) then
		...
		elseif self._owner or gstate._police[u_key] and gstate._police[u_key].is_converted or gstate:is_enemy_converted_to_criminal(unit) then
			...
			self._nickname = u_base.joker_name
			if not self._nickname and u_base.kpr_minion_owner_peer_id and Keepers and Keepers.get_joker_name_by_peer then
				self._nickname = Keepers:get_joker_name_by_peer(u_base.kpr_minion_owner_peer_id)
			end
